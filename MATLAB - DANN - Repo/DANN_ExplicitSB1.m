...
SB1s2 = net.outputs{(archLayer+1)}.processSettings{1};
...
save('SB1structOW.mat', 'SB1s2')

save('SB1IW1.txt','IW','-ascii')
save('SB1LW1.txt','LW','-ascii')
save('SB1LW2.txt','LW2','-ascii')

save('SB1b1.txt','b1','-ascii')
save('SB1b2.txt','b2','-ascii')
save('SB1b3.txt','b3','-ascii')

save('SB1norm.txt', 'normFactor','-ascii')

% Xtest = [2.847 1.593 1.224 0.604 1.072 1.390 2.165 0.413]';
% modelTest = net(Xtest)*normFactor(2)+normFactor(1); % neural net output value
